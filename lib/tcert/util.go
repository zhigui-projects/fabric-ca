...
func GetCertificate(certificate []byte) (*x509.Certificate, error) {
	...
	if block == nil {
		//if util.GAlgorithm == "SM2"{
		if gm.IsGM(){
			certificates, err = gmx509.X509(gmx509.SM2).ParseCertificates(certificate)
		}else{
		...
		//if util.GAlgorithm == "SM2"{
		if gm.IsGM(){
			certificates, err = gmx509.X509(gmx509.SM2).ParseCertificates(block.Bytes)
		}else{
...
func ParsePrivateKey(der []byte) (interface{}, error) {
	...
	key,err := sm2.ParsePKCS8UnecryptedPrivateKey(der)
	if err != nil {
